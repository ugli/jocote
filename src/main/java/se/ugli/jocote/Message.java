...
public interface Message {
    ...
    default Optional<Object> header(String name) {
        return Optional.ofNullable(headers().get(name));
    }

    default Optional<Integer> headerAsInteger(String name) {
        return header(name).flatMap(v -> Optional.of(Integer.parseInt(v.toString())));
    }

    default int headerAsInteger(String name, int defaultValue) {
        return headerAsInteger(name).orElse(defaultValue);
    }

    default Optional<Long> headerAsLong(String name) {
        return header(name).flatMap(v -> Optional.of(Long.parseLong(v.toString())));
    }

    default long headerAsLong(String name, long defaultValue) {
        return headerAsLong(name).orElse(defaultValue);
    }
    ...
    static class MessageImpl implements Message {
        ...
        MessageImpl(final String id, final byte[] body, final Map<String, Object> headers,
        ...
        public String toString() {
            return "MessageImpl [id=" + id + ", headers=" + headers + ", properties=" + properties + ", bodySize="
                    + body.length + "]";
        }
